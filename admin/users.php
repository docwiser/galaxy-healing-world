...
<span>
    ...
    ${user.dob ? `<span class="dob-status">(${user.is_dob_approximate == 1 ? 'Approximate' : 'Known'})</span>` : ''}
</span>
</div>
 <div class="detail-item">
...
<div class="detail-item full-width" hidden style="display:none;">
    ...
    <div class="detail-grid">
        ...
        <div class="detail-item full-width">
            ...
            <div class="document-list" style="margin-top: 8px;">
                <a href="/uploads/disability_docs/${user.disability_certificate}" target="_blank" class="document-link" style="display: inline-flex; align-items: center; margin-right: 15px; text-decoration: none; color: #2563eb; background: #eff6ff; padding: 8px 12px; border-radius: 6px; font-size: 14px;">
                    <i data-feather="file" style="width: 16px; height: 16px; margin-right: 8px;"></i>
        ...
        ${(() => {
                    let docs = [];
                    try {
                        if (user.disability_documents) {
                            docs = JSON.parse(user.disability_documents);
                        }
                    } catch (e) {
                        console.error('Error parsing disability documents', e);
                    }

                    if (docs && docs.length > 0) {
                        return `
                <div class="detail-item full-width">
                    <label>Disability Certificates:</label>
                    <div class="document-list" style="margin-top: 8px;">
                        ${docs.map((doc, index) => {
                            const fileName = doc.split('/').pop();
                            return `
                                <a href="/${doc}" target="_blank" class="document-link" style="display: inline-flex; align-items: center; margin-right: 15px; text-decoration: none; color: #2563eb; background: #eff6ff; padding: 8px 12px; border-radius: 6px; font-size: 14px;">
                                    <i data-feather="file" style="width: 16px; height: 16px; margin-right: 8px;"></i>
                                    <span>Certificate ${index + 1} (${fileName.substring(0, 15)}${fileName.length > 15 ? '...' : ''})</span>
                                </a>
                            `;
                        }).join('')}
                    </div>
